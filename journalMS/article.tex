...
\Abstract{
\RBnote{Perhaps briefly state what the KS test is.}
The versatility of the univariate Kolmogorov-Smirnov (KS) test has
...
distribution-free~\citep{Babu2006}. Furthermore, the \proglang{C}
...
the current 2-D KS test on the Comprehensive \proglang{R} Archive
Network (CRAN): (i) the Fasano and Franceschini test has been shown to
run in $O(n^2)$ versus the Peacock implementation which runs in
...
%While a comprehensive discussion pertaining the intricacies of the 1-D KS test can be found elsewhere~\citep{Stephens1992a}, we will present a brief overview as motivation for the basis and intuition of the 2-D KS test.
%
...
%A $p$-value $< 0.05$, indicates a significant difference between the samples.
