...
is available in [@R] with the package via the function, but this has
been shown to be markedly slower than the Fasano and Franceschini
algorithm [@Lopes2007]. A implementation of the Fasano--Franceschini
...
distribution-free [@Babu2006]. Furthermore, in the implementation,
...
Here we present the package as an implementation of the 2-D two-sample
KS test described by Fasano and Franceschini [@Fasano1987]. The package
...
The R package implements a parallelized bootstrapping procedure. The
marginal distribution from 2-dimensional data set is resampled with
replacement to generate randomized 2-dimensional data sets times. The
...
Fasano--Franceschini test can be run as follows (see for further source
...
fasano.franceschini.test(S1 = sample1Data, S2 = sample2Data, nBootstrap = 1000, cores = 2)

```

To improve run time, one may adjust the parameter; see the R package and
...
follows. Using the package to evaluate runtime, the Fasano--Franceschini
...
the implentation of the bootstrapping procedure increases runtime in
...
The package is an implementation of the 2-D two-sample KS test as
...
Archive Network (CRAN) at <https://CRAN.R-project.org/> and the package
website at <https://nesscoder.github.io/fasano.franceschini.test/>.

# Computational details {#computational-details .unnumbered}

The results in this paper were obtained using 4.0.3 with the 1.0.0
package. itself and all package dependencies (4.0.3; 4.0.3) are
...
E.N.C developed the package and produced the tutorials/documentation;
